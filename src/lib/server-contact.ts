...
export const submitContactForm = createServerFn({ method: "POST" })
  .validator((data: unknown) => {
    const body = data as ContactFormData;

    // Honeypot check
    if (body.honeypot && body.honeypot.trim().length > 0) {
      return { isSpam: true } as any;
    }
    ...
    if (!name || name.length > 100) {
      throw new Error("Please provide a valid name under 100 characters.");
    }
    ...
    if (!projectType) {
      throw new Error("Please select a service / project type.");
    }

    if (!message || message.length > 3000) {
      throw new Error("Please provide a message under 3000 characters.");
    }
    ...
    return {
      ...
      budget,
      message,
    } as ContactFormData & { isSpam?: boolean };
  })
  .handler(async ({ data }) => {
    if (data.isSpam) {
    ...
    const { name, email, phone, company, subject, projectType, budget, message } = data;
...
`;
    ...
    if (!sent) {
      if (!resendApiKey && !process.env.SMTP_USER && !process.env.SMTP_HOST && !process.env.WEBHOOK_EMAIL_URL) {
        console.error(
          "[Email] No email service configured. Please set RESEND_API_KEY (recommended) or SMTP_USER & SMTP_PASS.",
        );
        throw new Error(
          "Email service configuration required. Please configure RESEND_API_KEY or SMTP credentials in your environment variables.",
        );
      }

      throw new Error(lastError?.message || "Failed to send email through configured service.");
    }
